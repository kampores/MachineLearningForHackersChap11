...
twitter.network<-function(user){
  ego<-getUser(user)
  friends.list<-sapply(ego$getFollowers(n=100),function(x) c(x$screenName))
  if(length(friends.list)>0){
    friends<-cbind(ego$screenName, unname(friends.list))
  }
  else{
    friends<-c(integer(0),integer(0))
  }
  followers.list<-sapply(ego$getFollowers(n=100),function(x) c(x$screenName))
  if(length(followers.list)>0){
    followers<-cbind(unname(followers.list),ego$screenName)
  }
  else{
...
twitter.snowball <- function(seed, k=2) {
    ...
    while(rounds < k) {
        ...
        for(user in new.seeds) {
            ...
            # 이 노드에 방문한 적이 없는 경우에만 네트워크 데이터를 얻는다.
    ...
    # 이 과정으로 중복 행이 생길 수 있다.
    # 실제 트위터 소셜 네트워크레서는 하나의 연결이 중복되는 경우가 없으므로,
    # 이 행들을 제거해준다.
