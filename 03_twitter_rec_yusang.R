...
# neighbors 함수로 이웃 노드의 인덱스를 구한다.
# 노드 속성값을 반환하는 V함수에 이 값을 인수로 넘겨서 name 속성값을 구한다.
# get.edgelist 함수를 이용해서 네트워크 전체 링크 목록을 N x 2행렬로 받는다.

friends <- V(user.graph)$name[neighbors(user.graph, user, mode = "out") ] # remove +1
...
# ifelse에서 첫 테스트는 행의 원소 중에 그 사용자가 있는지와 첫 원소(링크출발점)가 시작점의 친구가 아닌지를 검사하여 조건이 맞으면 그 행은 무시한다.
# 두번째 테스트는 행의 두번째 원소(링크의 끝점)가 시작점의 친구가 아닌지를 확인한다.
...
# Report the results
# table 함수가 생성한 벡터에서 데이터프레임을 만든다.
# 시작점의 친구 중 몇%가 각 추천 후보들을 필로잉했는지 계산해서
# 가장 추천하기 좋은 사용자를 평가하는 정규화 측도도 추가한다.
...
#데이터프레임을 각 사용자를 팔로잉하는 친구의 비율이 가장 높은 순으로 정렬한다.
...
# 각 분할에 속하는 사용자를 찾고, 원 사용자의 친구들 중에 가장 팔로워가 많은 사람을 찾아서 반환한다.
...
# 모든 파티션에 대해 partition.follow를 실행하고 NA와 중복을 제거한다.
...
# 모든 그래프의 노드인덱스를 얻고 추천유저를 더한다
new.friends <- as.character(c(V(user.ego)$name, partition.recs[,2]))
new.index <- match(new.friends, V(user.graph)$name )  #remove -1

# Take a new subgraph, which includes the new reccommendations
# 새 추천을 포함한 새 서브그래프를 만든다.
partition.graph <- induced.subgraph(user.graph, new.index) # subgraph to induced.subgraph
...
all.index <- match(as.character(all.partition[,2]), V(partition.graph)$name)  #remove -1
